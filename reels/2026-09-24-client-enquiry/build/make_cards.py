...
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "cards0924")
...
def card(lines, fontsize, center_y, name, pad_x=54, pad_y=46, radius=34,
    ...
    img.save(os.path.join(OUT,name)); print(f"{name}: {box_w}x{box_h}  y {y0}-{y0+box_h}")

# Hook verbatim from the brief. Round 6 ends on the output, so no end card.
# y 620 keeps it clear of the composer and the send button during the tap.
card(["Every client sends you","this paragraph."], 62, 620, "hook.png")
